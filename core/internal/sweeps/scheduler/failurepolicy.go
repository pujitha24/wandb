...
// a W&B API call.
//
// The HTTP client retries what is worth retrying — timeouts, connection
// failures, 5xx — save for the rate limit schedulerRetryPolicy hands
// straight back. So an error that gets this far is either a 429 for the
// loop's own backoff to absorb, or one that has already outlived those
// retries and will not start succeeding because the loop calls again.
// Everything in the second group is fatal, which lets the step discard
// whatever the call was going to schedule instead of leaving it in limbo.
...
// doubling slowdown, which is what the server asked for. Twenty quick
// retries inside the one call would instead spend the whole rate-limit
// window making the problem worse, and leave the loop a bare error with
// no status to slow down on.
//
